...
public:
    ...
    /// max_time: optional max timestamp from part data (0 = not provided, will parse from partition_id for TTL cache)
    virtual void set(const String & key, ReadBuffer & value, size_t weight_hint, bool is_preload, time_t max_time = 0) = 0;
...
public:
    ...
    virtual void set(const String &, ReadBuffer &, size_t, bool, time_t = 0) override { throw Exception("MultiDiskCache `set` is not supported now", ErrorCodes::LOGICAL_ERROR);}
